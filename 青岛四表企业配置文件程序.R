...
fourtables_datapath<-"D://工作//纳税评估模型//2015年//2015.09.16青岛纳税评估模型运算//2015.09.16青岛数据//1、四表齐全//"
zzsb<-read.csv(paste(fourtables_datapath,"增值税申报表四表齐全.csv",sep=""),colClasses=c(rep("character",2)))
sdsb<-read.csv(paste(fourtables_datapath,"年度所得税申报表.csv",sep=""),colClasses=c(rep("character",2)))
nsrxxb<-read.csv(paste(fourtables_datapath,"纳税人信息表.csv",sep=""),colClasses=c(rep("character",2)))
zcfzbnc<-read.csv(paste(fourtables_datapath,"资产负债表年初.csv",sep=""),colClasses=c(rep("character",2)))
zcfzbnm<-read.csv(paste(fourtables_datapath,"资产负债表年末.csv",sep=""),colClasses=c(rep("character",2)))
lrb<-read.csv(paste(fourtables_datapath,"利润表.csv",sep=""),colClasses=c(rep("character",2)))
